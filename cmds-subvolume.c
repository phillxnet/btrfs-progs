			     ...
			     size_t *capacity)
{
	...
	err = btrfs_util_create_subvolume_iterator_fd(fd, tree_id, 0, &iter);
	if (err) {
		iter = NULL;
		error_btrfs_util(err);
		goto out;
...
static struct subvol_list *btrfs_list_subvols(int fd, int is_list_all,
		const char *path, struct btrfs_list_filter_set_v2 *filter_set)
{
	struct subvol_list *subvols;
	size_t capacity = 0;
