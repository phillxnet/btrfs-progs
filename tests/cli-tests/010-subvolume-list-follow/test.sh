...
# test if the ids returned by "subvol list" match expected ids
# $1  ... option for subvolume list
# $2  ... PATH to be specified by subvol list command
...
test_list()
{
	...
	if [ "$result" != "$expected" ]; then
		...
		_fail "ids returned by subvol list do not match expected ids"
	fi
}
